import { Github, KeyRound } from "lucide-react";
import { useEffect, useState } from "react";
import { useSearchParams, useNavigate } from "react-router-dom";
import { Button } from "@/components/ui/button";
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from "@/components/ui/card";
import { Input } from "@/components/ui/input";
import { BrainCloudLogo } from "@/components/brand/logo";
import { MARKETING_URL, API_BASE } from "@/lib/config";

const errorMessages: Record<string, string> = {
  invalid_state: "Authentication failed (state mismatch). Please try again.",
  oauth_failed: "OAuth authentication failed. Please try again.",
  no_code: "No authorization code received. Please try again.",
};

interface AuthProviders {
  providers: string[];
  api_key_only: boolean;
}

export function LoginPage() {
  const [params] = useSearchParams();
  const navigate = useNavigate();
  const error = params.get("error");

  const [providers, setProviders] = useState<AuthProviders | null>(null);
  const [apiKey, setApiKey] = useState("");
  const [apiKeyError, setApiKeyError] = useState("");
  const [verifying, setVerifying] = useState(false);

  useEffect(() => {
    fetch(`${API_BASE}/auth/providers`)
      .then((r) => r.json())
      .then((data: AuthProviders) => setProviders(data))
      .catch(() => {
        // Fallback: assume both providers available (pre-update backend)
        setProviders({ providers: ["github", "google"], api_key_only: false });
      });
  }, []);

  const handleApiKeySubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    const key = apiKey.trim();
    if (!key) return;

    setApiKeyError("");
    setVerifying(true);

    try {
      const res = await fetch(`${API_BASE}/api/health`, {
        headers: { "X-API-Key": key },
      });
      if (res.ok) {
        localStorage.setItem("brain_api_key", key);
        navigate("/");
      } else {
        setApiKeyError("Invalid API key. Please check and try again.");
      }
    } catch {
      setApiKeyError("Could not reach the server. Please try again.");
    } finally {
      setVerifying(false);
    }
  };

  const hasGithub = providers?.providers.includes("github");
  const hasGoogle = providers?.providers.includes("google");
  const hasOAuth = hasGithub || hasGoogle;

  return (
    <div className="flex min-h-screen flex-col items-center justify-center bg-background">
      <Card className="w-full max-w-sm">
        <CardHeader className="text-center">
          <div className="mx-auto mb-2">
            <BrainCloudLogo size={48} className="text-primary" />
          </div>
          <CardTitle className="text-2xl">Brain Cloud</CardTitle>
          <CardDescription>
            {providers?.api_key_only
              ? "Enter your API key to continue"
              : "Sign in to your knowledge dashboard"}
          </CardDescription>
        </CardHeader>
        <CardContent className="flex flex-col gap-3">
          {error && (
            <div className="rounded-md border border-destructive/50 bg-destructive/10 p-3 text-sm text-destructive">
              {errorMessages[error] ?? `Authentication error: ${error}`}
            </div>
          )}

          {/* Loading state */}
          {!providers && (
            <div className="text-center text-sm text-muted-foreground py-2">
              Loading...
            </div>
          )}

          {/* OAuth buttons */}
          {hasGithub && (
            <Button asChild className="w-full">
              <a href="/auth/github">
                <Github className="mr-2 h-4 w-4" />
                Continue with GitHub
              </a>
            </Button>
          )}
          {hasGoogle && (
            <Button asChild variant="outline" className="w-full">
              <a href="/auth/google">
                <svg className="mr-2 h-4 w-4" viewBox="0 0 24 24">
                  <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 0 1-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z" fill="#4285F4" />
                  <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853" />
                  <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05" />
                  <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335" />
                </svg>
                Continue with Google
              </a>
            </Button>
          )}

          {/* Separator when both OAuth and API key are shown */}
          {hasOAuth && providers?.api_key_only === false && null}

          {/* API key form — always shown in api_key_only mode */}
          {providers?.api_key_only && (
            <form onSubmit={handleApiKeySubmit} className="flex flex-col gap-3">
              <Input
                type="password"
                placeholder="brain_..."
                value={apiKey}
                onChange={(e) => setApiKey(e.target.value)}
                autoFocus
              />
              {apiKeyError && (
                <p className="text-sm text-destructive">{apiKeyError}</p>
              )}
              <Button type="submit" disabled={!apiKey.trim() || verifying} className="w-full">
                <KeyRound className="mr-2 h-4 w-4" />
                {verifying ? "Verifying..." : "Continue with API Key"}
              </Button>
            </form>
          )}
        </CardContent>
      </Card>
      <a
        href={MARKETING_URL}
        className="mt-4 text-sm text-muted-foreground hover:text-foreground transition-colors"
      >
        &larr; Back to home
      </a>
    </div>
  );
}
